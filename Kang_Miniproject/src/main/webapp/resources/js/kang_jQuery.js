function connectSearchAddressEvent() {
	$("#join_k_addr1, #join_k_addr2").click(function() {
		new daum.Postcode({
			oncomplete : function(data) {
				$("#join_k_addr1").val(data.zonecode);
				$("#join_k_addr2").val(data.roadAddress);
			}
		}).open();
	});
}
...
function connectIdCheckEvent() {
	$("#join_k_id").keyup(function() {
		var k_id = $(this).val();
		$.getJSON("member.id.check?k_id="+k_id, function(memberJSON){
			if (memberJSON.member[0] == null) {
				$("#join_k_id").css("color", "black");
			} else {
				$("#join_k_id").css("color", "red");
			}
		});
	});
}
